...
const parseMovieYifi = (idImdb, torrents, movieDb, imdbApi) => {
  // console.log('moviedbEN ', movieDb.en);
  // console.log('moviedbFR ', movieDb.fr);
  ...
  let year = parseInt(imdbApi.year, 10);
  if (isNaN(year)) year = -1;
  let runtime = parseInt(imdbApi.length, 10);
  if (isNaN(runtime)) runtime = -1;
  let rating = parseFloat(imdbApi.rating);
...
const FetchTheMovieDBInfo = async (idImdb, lang) => {
  const url = urlMovieDb(idImdb, lang);
  //console.log(url);
  return axios.get(url)
    .then(({ data }) => (
      data.movie_results[0]
    ));
  // .catch(err => console.error('FetchTheMovieDBInfo err', err));
};
